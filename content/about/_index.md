---
title: "About"
date: 2026-08-10
---

I’m **Erik Xu**—a software enthusiast and writer building simple things for the web.

This site serves as my digital notebook: a place to collect fleeting thoughts, document small projects, and log things I’m learning along the way.

---

### What I’m Doing Now

{{< currently >}}

*(This section is inspired by Derek Sivers’ [/now page](https://nownownow.com/about) movement).*

---

### Colophon / Tech Stack

This website is deliberately simple and low-overhead:

* **Engine:** [Hugo](https://gohugo.io/) (Static Site Generator)
* **Theme:** [Risotto](https://github.com/joerick/risotto) by Joe Rick
* **Hosting:** [GitHub Pages](https://pages.github.com/)
* **Typography:** System fonts for instant load times and zero tracking

---

### Get in Touch

Feel free to reach out via [Linkedin](https://www.linkedin.com/in/erikxxu/) or find my code on [GitHub](https://github.com).
